...
export class WeatherService {
  ...
  async getAllDbCities (): Promise<CityCoordinates[]> {
    ...
    if (error) {
      console.error('Error fetching all cities from DB:', error)
      throw new Error(
        `Failed to fetch all cities from database: ${error.message}`,
      )
    }
  ...
  // Fetches weather data for all cities in our database
  async getAllDbCitiesWeather (): Promise<City[]> {
    const allCityCoordinates = await this.getAllDbCities()
  ...
  async insertNewCity (
    ...
    if (error) {
      if (error.code === '23505') {
        // unique_violation, city already exists
